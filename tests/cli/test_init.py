...
def test_main_set_vars(runner: CliRunner, mock_config_file: Path) -> None:
    ...
    with patch("persona.storage.local.LocalStorageBackend.load") as mock_local_storage_load:
        mock_local_storage_load.return_value = Index(personas=SubIndex(root={}), skills=SubIndex(root={})).model_dump_json()
        # Act
...
def test_main_no_config_file(runner: CliRunner, mock_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # Arrange
