...
def mock_app_context() -> AppContext:
    ...
    mock_storage_config.root = (
        mock_local_storage_config  # StorageConfig.root is the AnyStorage object
    )
...
async def test_lifespan_from_config_file(tmp_path: plb.Path) -> None:
    ...
    mock_config_data = {'type': 'local', 'root': str(tmp_path), 'index': 'test_index'}
    expected_storage_config = StorageConfig.model_validate(mock_config_data)

    with (
        patch('persona.mcp.utils.os.environ', {'PERSONA_CONFIG_PATH': str(mock_config_path)}),
        patch('persona.mcp.utils.plb.Path.exists', return_value=True),
        patch(
            'persona.mcp.utils.plb.Path.open', MagicMock(return_value=mock_config_path.open('r'))
        ),
...
async def test_lifespan_from_env_vars() -> None:
    ...
    with (
        patch.dict('persona.mcp.utils.os.environ', {}, clear=True),
...
async def test_list_items(
    list_type: Literal['personas', 'skills'], mock_app_context: AppContext
) -> None:
...
async def test_write_skill_files_not_absolute_path(mock_app_context: AppContext) -> None:
    ...
    with pytest.raises(
        ToolError, match=r'Target skill directory \"relative/path\" is not an absolute path.'
    ):
        await _write_skill_files(mock_app_context, 'relative/path', 'test_skill')


@pytest.mark.asyncio
async def test_write_skill_files_dir_not_exists(
    mock_app_context: AppContext, tmp_path: plb.Path
) -> None:
    # Arrange: Mock AppContext
    ...
    with pytest.raises(
        ToolError,
        match=r'Target skill directory ".*" does not exist\. Please create it before installing the skill\.',
    ):
        await _write_skill_files(mock_app_context, str(non_existent_path), 'test_skill')


@pytest.mark.asyncio
async def test_write_skill_files_builtin_skill(
    tmp_path: plb.Path, mock_app_context: AppContext
) -> None:
    ...
    with patch('persona.mcp.utils.library_skills', {'builtin_skill': mock_skill_files}):
        ...
        skill_file_path = await _write_skill_files(mock_app_context, str(tmp_path), 'builtin_skill')
...
@pytest.mark.asyncio
async def test_write_skill_files_remote_skill(
    tmp_path: plb.Path, mock_app_context: AppContext
) -> None:
    ...
    with patch('persona.mcp.utils._skill_files', AsyncMock(return_value=mock_skill_files)):
        ...
        skill_file_path = await _write_skill_files(mock_app_context, str(tmp_path), 'remote_skill')
...
@pytest.mark.asyncio
async def test_write_skill_files_no_skill_md(
    tmp_path: plb.Path, mock_app_context: AppContext
) -> None:
    ...
    with patch('persona.mcp.utils._skill_files', AsyncMock(return_value=mock_skill_files)):
        ...
        with pytest.raises(
            ToolError,
            match=r'SKILL\.md file not found for skill ".*?"\. Installation may have failed\.',
        ):
...
async def test_get_persona_found(mock_app_context: AppContext) -> None:
    ...
    mock_app_context._vector_db.exists.return_value = True
    mock_app_context._target_storage.load.return_value = (
...
async def test_match_items(
    match_type: Literal['personas', 'skills'], mock_app_context: AppContext
) -> None:
    # Arrange: Mock AppContext and VectorDatabase.search
    mock_table = MagicMock()
    mock_table.to_arrow.return_value.select.return_value.to_pylist.return_value = [
        {'name': 'match', 'description': 'desc', 'uuid': '123', '_distance': 0.5}
    ]
    mock_app_context._vector_db.search.return_value = mock_table

    # Act: Call _match
    result = await _match(
        match_type, 'test query', mock_app_context, limit=1, max_cosine_distance=0.8
    )
