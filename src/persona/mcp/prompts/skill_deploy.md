...
3. **Skill Retrieval**: Skill Verification & Acquisition: After `match_skill` returns a skill, you MUST follow this exact sequence:
   ...
   b. Conditional Retrieval:
         - If the skill directory exists, you are forbidden from calling get_skill. Proceed directly to the Execution Plan (Step 5) using the local files.
         - If the skill directory does not exist, you must then call `get_skill` to retrieve it.
