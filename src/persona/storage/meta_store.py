...
import os
import logging
from typing import Literal, TYPE_CHECKING, Self, Generic, TypeVar, Generator, Protocol, Any
...
class MetaStore:
    ...
    def __init__(self, cursor: CursorLike):
        self._cursor = cursor
        self._logger = logging.getLogger(f"persona.storage.meta_store.MetaStore")
...
class MetaStoreEngine(Generic[T], metaclass=ABCMeta):
    ...
    @contextmanager
    def session(self) -> Generator[MetaStore, None, None]:
        self._logger.debug("Starting new metastore session ...")
        cursor = self.get_cursor()
        cursor.begin()
        try:
            yield MetaStore(cursor)
            cursor.commit()
...
class DuckDBMetaStoreEngine(MetaStoreEngine[DuckDBMetaStoreConfig]):
