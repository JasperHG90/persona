...
class BaseMetaStore(metaclass=ABCMeta):
    ...
    @abstractmethod
    def get_many(self, table_name: personaTypes, row_filter: list[str] | None = None, column_filter: list[str] | None = None) -> pa.Table:
...
class CursorLikeMetaStore(BaseMetaStore):
    ...
    def get_many(self, table_name: personaTypes, row_filter: list[str] | None = None, column_filter: list[str] | None = None) -> pa.Table:
        ...
        if row_filter is not None:
            sql += ' WHERE name = ANY(?)'
        return self._cursor.execute(sql, [] if row_filter is None else [row_filter]).fetch_arrow_table()
